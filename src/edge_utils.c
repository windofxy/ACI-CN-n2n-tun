...
#define N2N_TCP_FALLBACK_KCP_PROBE_INTERVAL 5
...
void reset_sup_attempts (n2n_edge_t *eee) {
    ...
    eee->sup_attempts = edge_transport_uses_tcp(eee) ? 1 : N2N_EDGE_SUP_ATTEMPTS;
}


// detect local IP address by probing a connection to the supernode
static int detect_local_ip_address (n2n_sock_t* out_sock, const n2n_edge_t* eee, SOCKET base_sock) {
    ...
    if((base_sock >= 0)
    && (getsockname(base_sock, (struct sockaddr *)&local_sock, &sock_len) == 0)
...
static int edge_transport_is_forced_tcp (const n2n_edge_t *eee) {

    return (eee && eee->conf.connect_tcp);
}
...
static int edge_tcp_fallback_is_establishing (const n2n_edge_t *eee) {

    return (eee
         && eee->tcp_fallback_active
         && edge_transport_uses_tcp(eee)
         && eee->sn_wait);
}


static void edge_reset_register_super_request (n2n_edge_t *eee) {
...
static const char *edge_supernode_rx_transport_str (uint8_t transport) {

    switch(transport) {
        case N2N_RX_SUPERNODE_TRANSPORT_UDP: return "UDP";
        case N2N_RX_SUPERNODE_TRANSPORT_TCP: return "TCP";
        case N2N_RX_SUPERNODE_TRANSPORT_KCP: return "KCP";
        default: return "UNKNOWN";
    }
}


static int edge_recent_supernode_udp_activity (const n2n_edge_t *eee, time_t now) {

    if(!eee || !eee->last_udp_sup)
        return 0;

    return ((now - eee->last_udp_sup) <= N2N_SUPERNODE_UDP_ACTIVITY_GRACE_SECS);
}
...
static void edge_sync_active_socket (n2n_edge_t *eee) {
    ...
    if(edge_transport_uses_tcp(eee))
        eee->sock = eee->tcp_sock;
    else
        eee->sock = eee->udp_sock;
}
...
static int edge_dest_is_current_supernode (const n2n_edge_t *eee, const n2n_sock_t *sock) {

    return (eee && sock && eee->curr_sn && sock_equal(sock, &(eee->curr_sn->sock)));
}


static void supernode_disconnect_udp (n2n_edge_t *eee) {
...
static void supernode_disconnect_tcp (n2n_edge_t *eee) {

    if(!eee)
        return;

    if(eee->tcp_sock >= 0) {
        closesocket(eee->tcp_sock);
        eee->tcp_sock = -1;
        traceEvent(TRACE_DEBUG, "closed TCP socket");
    }

    memset(&eee->tcp_sn_sock, 0, sizeof(n2n_sock_t));
    eee->tcp_sn_sock.family = (uint8_t)AF_INVALID;
    eee->tcp_sn_sock_valid = 0;
}


static void edge_release_tcp_sticky_after_disconnect (n2n_edge_t *eee, const char *reason) {

    if(!eee || edge_transport_is_forced_tcp(eee) || !eee->tcp_sticky_after_fallback)
        return;

    if(reason)
        traceEvent(TRACE_INFO, "TCP fallback ended: %s", reason);

    eee->tcp_sticky_after_fallback = 0;
...
static void edge_mark_udp_recovered (n2n_edge_t *eee, const char *reason) {
    ...
    if(eee->tcp_sticky_after_fallback) {
        if(reason)
            traceEvent(TRACE_INFO, "UDP/KCP traffic from supernode was seen again, but edge is pinned to TCP fallback until the TCP connection breaks: %s", reason);
        return;
    }
...
static int edge_register_retry_interval_ms (const n2n_edge_t *eee) {

    static const int retry_schedule_ms[] = { 300, 600, 1000, 1500 };
    uint8_t stage;
    size_t max_stage;

    if(!eee)
        return retry_schedule_ms[0];

    stage = eee->register_fast_retry_count;
    max_stage = (sizeof(retry_schedule_ms) / sizeof(retry_schedule_ms[0])) - 1;

    if(stage > max_stage)
        stage = (uint8_t)max_stage;

    return retry_schedule_ms[stage];
}


static int edge_register_retry_wait_ms (const n2n_edge_t *eee, uint32_t current_ms, int default_ms) {
    ...
    if(eee->sn_wait == 2 || eee->last_register_req_ms == 0)
        return 0;

    interval_ms = edge_register_retry_interval_ms(eee);
    elapsed_ms = current_ms - eee->last_register_req_ms;

    if(elapsed_ms >= (uint32_t)interval_ms)
        return 0;

    interval_ms -= (int)elapsed_ms;

    if(interval_ms > default_ms)
        return default_ms;
...
static void edge_handle_dead_kcp_session (n2n_edge_t *eee) {
...
static void supernode_apply_socket_options (n2n_edge_t *eee, SOCKET sockfd) {
    ...
    if(eee->conf.tos) {
        ...
        if(setsockopt(sockfd, IPPROTO_IP, IP_TOS, (char *)&sockopt, sizeof(sockopt)) == 0)
    ...
    if(setsockopt(sockfd, IPPROTO_IP, IP_MTU_DISCOVER, &sockopt, sizeof(sockopt)) < 0)
...
static void supernode_update_local_socket (n2n_edge_t *eee, SOCKET sockfd) {

    n2n_sock_t local_sock;
    n2n_sock_str_t sockbuf;

    memset(&local_sock, 0, sizeof(n2n_sock_t));
    if(detect_local_ip_address(&local_sock, eee, sockfd) == 0) {
...
static int supernode_connect_udp_socket (n2n_edge_t *eee) {
    ...
    eee->udp_sock = open_socket(eee->conf.local_port, eee->conf.bind_address, 0);
    if(eee->udp_sock < 0) {
        traceEvent(TRACE_ERROR, "failed to bind main UDP port %u", eee->conf.local_port);
        return -1;
    }

    supernode_apply_socket_options(eee, eee->udp_sock);
    supernode_update_local_socket(eee, eee->udp_sock);
...
static int supernode_connect_tcp_socket (n2n_edge_t *eee) {
    ...
    if((eee->tcp_sock >= 0)
    && eee->tcp_sn_sock_valid
    && edge_dest_is_current_supernode(eee, &eee->tcp_sn_sock))
        return 0;

    supernode_disconnect_tcp(eee);

    if(eee->conf.local_port > 0)
        traceEvent(TRACE_NORMAL, "binding to local port %d", 0);

    eee->tcp_sock = open_socket(0, eee->conf.bind_address, 1);
    if(eee->tcp_sock < 0) {
        traceEvent(TRACE_ERROR, "failed to bind main UDP port %u", 0);
        return -1;
    }
    ...
    {
        u_long value = 1;
        ioctlsocket(eee->tcp_sock, FIONBIO, &value);
        if(connect(eee->tcp_sock, (struct sockaddr*)&sn_sock, sizeof(struct sockaddr)) < 0) {
...
#else
    fcntl(eee->tcp_sock, F_SETFL, O_NONBLOCK);
    if((connect(eee->tcp_sock, (struct sockaddr*)&sn_sock, sizeof(struct sockaddr)) < 0)
       && (errno != EINPROGRESS)) {
    ...
    memcpy(&eee->tcp_sn_sock, &eee->curr_sn->sock, sizeof(n2n_sock_t));
    eee->tcp_sn_sock_valid = 1;
...
int supernode_connect (n2n_edge_t *eee) {
    ...
    if(!edge_transport_is_forced_tcp(eee) && (supernode_connect_udp_socket(eee) < 0))
        return -1;

    if(edge_transport_is_forced_tcp(eee) || eee->tcp_fallback_active) {
        if(supernode_connect_tcp_socket(eee) < 0)
            return -1;
        eee->active_sn_transport = N2N_SN_TRANSPORT_TCP;
    } else {
        eee->active_sn_transport = N2N_SN_TRANSPORT_UDP;
    }
...
int edge_switch_to_tcp_supernode (n2n_edge_t *eee, const char *reason) {
    ...
    traceEvent(TRACE_WARNING,
               "UDP/KCP transport to supernode [%s] appears unavailable, falling back to TCP",
               supernode_ip(eee));
    if(reason)
        traceEvent(TRACE_WARNING, "transport fallback detail: %s", reason);

    eee->tcp_fallback_active = 1;
    eee->tcp_sticky_after_fallback = 1;
    eee->kcp_probe_pending = 0;
    eee->kcp_probe_cookie = 0;
    eee->last_kcp_probe = 0;
    eee->register_fast_retry_count = 0;
    eee->tcp_register_soft_retry_budget = N2N_TCP_REGISTER_SOFT_RETRIES;
    eee->register_super_soft_retry_armed = 0;
    ...
    edge_set_active_transport(eee, N2N_SN_TRANSPORT_TCP);
    reset_sup_attempts(eee);
    eee->sn_wait = 0;
    return 1;
#else
...
n2n_edge_t* edge_init (const n2n_edge_conf_t *conf, int *rv) {
    ...
    eee->tcp_sn_sock.family = (uint8_t)AF_INVALID;
    eee->tcp_sn_sock_valid = 0;
    eee->active_sn_transport = eee->conf.connect_tcp ? N2N_SN_TRANSPORT_TCP : N2N_SN_TRANSPORT_UDP;
    eee->tcp_fallback_active = 0;
    eee->tcp_sticky_after_fallback = 0;
    eee->kcp_probe_pending = 0;
    eee->kcp_probe_cookie = 0;
...
err_out:
    if(edge_transport_uses_tcp(eee)) {
        if(eee->sending_register_super
        && eee->tcp_fallback_active
        && !eee->register_super_soft_retry_armed
        && eee->tcp_register_soft_retry_budget > 0) {
            unsigned int retries_left;
            const char *transport = (eee->active_sn_transport == N2N_SN_TRANSPORT_TCP) ? "TCP"
                                  : ((eee->active_sn_transport == N2N_SN_TRANSPORT_UDP) ? "UDP" : "UNKNOWN");

            eee->register_super_soft_retry_armed = 1;
            eee->tcp_register_soft_retry_budget--;
            retries_left = (unsigned int)eee->tcp_register_soft_retry_budget;

            traceEvent(TRACE_WARNING,
                       "TCP REGISTER_SUPER send failed during fallback to supernode [%s], scheduling soft retry (%u retries left, transport=%s)",
                       supernode_ip(eee),
                       retries_left,
                       transport);
#ifdef _WIN32
            traceEvent(TRACE_WARNING,
                       "TCP REGISTER_SUPER fallback send detail: send_failed=%d errno=%d (%s) wsa=%u",
                       send_failed, sock_errno, errstr ? errstr : "unknown", wsa_error);
#else
            traceEvent(TRACE_WARNING,
                       "TCP REGISTER_SUPER fallback send detail: send_failed=%d errno=%d (%s)",
                       send_failed, sock_errno, errstr ? errstr : "unknown");
#endif
            ...
            if(supernode_connect_tcp_socket(eee) == 0) {
                ...
                traceEvent(TRACE_WARNING,
                           "TCP soft retry reconnect to supernode [%s] could not be prepared immediately",
                           supernode_ip(eee));
            }

            eee->sn_wait = 2;
            eee->last_register_req_ms = n2n_kcp_now_ms();
            edge_sync_active_socket(eee);
            return -1;
        }

        supernode_disconnect_tcp(eee);
        edge_release_tcp_sticky_after_disconnect(eee, "send path to supernode over TCP failed");
        eee->sn_wait = 1;
        edge_sync_active_socket(eee);
...
static void sendto_sock (n2n_edge_t *eee, const void * buf,
    ...
    if(edge_tcp_fallback_is_establishing(eee) && !eee->sending_supernode_control) {
        traceEvent(TRACE_DEBUG,
                   "dropping non-control send to [%s] while TCP fallback to supernode [%s] is still establishing",
...
void send_register_super (n2n_edge_t *eee) {
    ...
    if(!eee->register_super_request_active) {
        eee->register_super_cookie = n2n_rand();
        get_local_auth(eee, &(eee->register_super_auth));
        eee->register_super_request_active = 1;
    }

    eee->curr_sn->last_cookie = eee->register_super_cookie;
    ...
    memcpy(&(reg.auth), &(eee->register_super_auth), sizeof(reg.auth));
...
static void send_register (n2n_edge_t * eee,
    ...
    traceEvent(TRACE_DEBUG, "send REGISTER to [%s]",
...
static void send_register_ack (n2n_edge_t * eee,
    ...
    traceEvent(TRACE_DEBUG, "send REGISTER_ACK to [%s]",
               sock_to_cstr(sockbuf, remote_peer));
...
void update_supernode_reg (n2n_edge_t * eee, time_t now) {
    ...
    int cnt = 0;
    int off = 0;
    int fast_retry_due = 0;
    int was_waiting = eee->sn_wait;
    uint32_t current_ms = n2n_kcp_now_ms();

    if(eee->sn_wait == 2) {
        fast_retry_due = 1;
    } else if(eee->sn_wait && eee->last_register_req_ms) {
        uint32_t elapsed_ms = current_ms - eee->last_register_req_ms;
        if(elapsed_ms >= (uint32_t)edge_register_retry_interval_ms(eee))
            fast_retry_due = 1;
    }

    if(fast_retry_due) {
        /* fall through */
        traceEvent(TRACE_DEBUG, "update_supernode_reg: doing fast retry (%d ms stage=%u).",
                   edge_register_retry_interval_ms(eee),
                   (unsigned int)eee->register_fast_retry_count);
    ...
    if(eee->tcp_fallback_active && edge_transport_uses_tcp(eee)) {
        if(!eee->tcp_sticky_after_fallback
        && ((eee->last_kcp_probe == 0) || (now >= (eee->last_kcp_probe + N2N_TCP_FALLBACK_KCP_PROBE_INTERVAL)))) {
            n2n_sn_transport_t saved_transport = (n2n_sn_transport_t)eee->active_sn_transport;

            eee->last_kcp_probe = now;
            edge_set_active_transport(eee, N2N_SN_TRANSPORT_UDP);
            send_register_super(eee);
            edge_set_active_transport(eee, saved_transport);
        }
    ...
    if(0 == eee->sup_attempts) {
        if(!edge_transport_is_forced_tcp(eee)
        && !edge_transport_uses_tcp(eee)
        && edge_recent_supernode_udp_activity(eee, now)) {
            traceEvent(TRACE_INFO,
                       "deferring TCP fallback for supernode [%s]: recent UDP traffic was still received",
                       supernode_ip(eee));
            eee->sn_wait = 0;
            eee->register_fast_retry_count = 0;
            eee->tcp_register_soft_retry_budget = 0;
            ...
            traceEvent(TRACE_INFO,
                       "deferring TCP fallback for supernode [%s]: recent confirmed KCP traffic was still received",
                       supernode_ip(eee));
            eee->sn_wait = 0;
            eee->register_fast_retry_count = 0;
            eee->tcp_register_soft_retry_budget = 0;
    ...
    if(edge_transport_uses_tcp(eee) && (eee->tcp_sock < 0)) {
        if(supernode_connect_tcp_socket(eee) < 0) {
            traceEvent(TRACE_WARNING, "TCP control-plane reconnect to supernode [%s] is not ready yet",
                       supernode_ip(eee));
            eee->sn_wait = 1;
            eee->last_register_req = now;
            eee->last_register_req_ms = current_ms;
            if(was_waiting && (eee->register_fast_retry_count < 3))
                eee->register_fast_retry_count++;
            return;
        }
        edge_set_active_transport(eee, N2N_SN_TRANSPORT_TCP);
    }
    ...
    eee->last_register_req_ms = current_ms;
    if(was_waiting) {
        if(eee->register_fast_retry_count < 3)
            eee->register_fast_retry_count++;
    } else {
        eee->register_fast_retry_count = 0;
    }
    ...
    /* Handle transform. */
    {
        ...
        if(rx_transop_id == eee->conf.transop_id) {
            ...
            if(eee->conf.drop_multicast && is_multicast) {
                traceEvent(TRACE_DEBUG, "dropping RX multicast");
                return(-1);
                ...
                if((ntohs(eh->type) == 0x0800) && (eth_size >= ETH_FRAMESIZE + IP4_MIN_SIZE)) {
                    ...
                    if(!memcmp(dst_mac, broadcast_mac, N2N_MAC_SIZE))
                        ...
                        traceEvent(TRACE_DEBUG, "discarding routed packet destined to [%s]",
                                   intoa(ntohl(*dst), ip_buf, sizeof(ip_buf)));
                        return(-1);
                    } else {
...
static int send_packet (n2n_edge_t * eee,
    ...
    if(edge_tcp_fallback_is_establishing(eee)) {
        traceEvent(TRACE_DEBUG,
                   "dropping encoded PACKET for %s while TCP fallback to supernode [%s] is still establishing",
    ...
    traceEvent(TRACE_DEBUG, "Tx PACKET of %u bytes to %s [%s]",
...
void process_udp (n2n_edge_t *eee, const struct sockaddr *sender_sock, const SOCKET in_sock,
    ...
    if((eee->tcp_sock >= 0) && (in_sock == eee->tcp_sock))
        ...
        switch(msg_type) {
            ...
            case MSG_TYPE_REGISTER: {
                ...
                if(!from_supernode) {
                     ...
                     */
                    traceEvent(TRACE_DEBUG, "[p2p] Rx REGISTER from %s [%s]%s",
            ...
            case MSG_TYPE_REGISTER_ACK: {
                ...
                traceEvent(TRACE_DEBUG, "Rx REGISTER_ACK from %s [%s] to %s via [%s]%s",
            ...
            case MSG_TYPE_REGISTER_SUPER_ACK: {
                ...
                if(!(eee->sn_wait)) {
                    /* Old or remote supernodes can still deliver one more ACK after we've
                     * already completed the registration round. Treat same-cookie ACKs as
                     * benign duplicates and drop them quietly to avoid misleading logs. */
                    if(ra.cookie == eee->curr_sn->last_cookie)
                        return;

                    traceEvent(TRACE_DEBUG, "Rx unexpected REGISTER_SUPER_ACK with no outstanding REGISTER_SUPER");
                    return;
                }
                ...
                if(ra.cookie != eee->curr_sn->last_cookie) {
                    traceEvent(TRACE_INFO,
                               "Rx REGISTER_SUPER_ACK with wrong or old cookie via %s (ack_cookie=%u current_cookie=%u)",
                               edge_supernode_rx_transport_str(eee->current_supernode_rx_transport),
                               (unsigned int)ra.cookie,
                               (unsigned int)eee->curr_sn->last_cookie);
                    return;
                }
...
int fetch_and_eventually_process_data (n2n_edge_t *eee, SOCKET sock,
    ...
    if(((eee->tcp_sock < 0) || (sock != eee->tcp_sock))
#ifndef SKIP_MULTICAST_PEERS_DISCOVERY
...
#endif
            supernode_disconnect_tcp(eee);
            edge_release_tcp_sticky_after_disconnect(eee, "TCP connection to supernode closed while receiving");
            eee->sn_wait = 1;
            edge_sync_active_socket(eee);
            goto tcp_done;
        }
            ...
            if(*position == sizeof(uint16_t)) {
                ...
                *expected = *expected + be16toh(*(uint16_t*)(pktbuf));
                if(*expected > N2N_PKT_BUF_SIZE) {
                    supernode_disconnect(eee);
                    edge_release_tcp_sticky_after_disconnect(eee, "TCP stream framing exceeded packet buffer while in fallback");
                    eee->sn_wait = 1;
                ...
                eee->current_supernode_rx_transport = N2N_RX_SUPERNODE_TRANSPORT_TCP;
    ...
    *eee->keep_running = true;
    ...
    while(*eee->keep_running) {
        ...
        if(eee->sn_kcp.active) {
            ...
            int wait_ms = edge_register_retry_wait_ms(eee, n2n_kcp_now_ms(),
                                                      (SOCKET_TIMEOUT_INTERVAL_SECS / 10 + 1) * 1000);
            wait_time.tv_sec = wait_ms / 1000;
            wait_time.tv_usec = (wait_ms % 1000) * 1000;
        } else {
        ...
        if(numPurged > 0) {
            traceEvent(TRACE_DEBUG, "%u peers removed. now: pending=%u, operational=%u",
                       numPurged,
                       HASH_COUNT(eee->pending_peers),
                       HASH_COUNT(eee->known_peers));
        }
